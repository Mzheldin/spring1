...
//        session.close();
        ...
        //удаление объекста (не работает)
//        ERROR: You have an error in your SQL syntax; check the manual that corresponds to your MySQL server version for the right syntax to use near 'where course_id=3' at line 1
        session = factory.getCurrentSession();
        session.beginTransaction();
        ...
        //получение дополнительной информации с помощью запросов(не работает)
//        session = factory.getCurrentSession();
//        session.beginTransaction();
...
//                .createQuery("from CoursesStudents cs where cs.student_id = :student_id and cs.course_id = :course_id")
